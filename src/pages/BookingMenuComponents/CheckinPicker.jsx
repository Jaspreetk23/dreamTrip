...
const CheckInPicker = ({ label, checkInDate, setCheckInDate }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedDate, setSelectedDate] = useState(checkInDate || new Date());
  const calendarRef = useRef(null);

  const handleDateChange = (date) => {
    const formattedDate = format(new Date(date), "dd MMM yy");
    setCheckInDate(formattedDate); // Pass formatted date to parent (clears check-out date)
